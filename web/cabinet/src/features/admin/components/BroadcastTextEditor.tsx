...
import { EXPANDABLE_CLASS, SPOILER_CLASS, safeHref, serializeToTelegramHtml } from '../utils/telegramHtml'

interface BroadcastTextEditorProps {
  /** Разметка Telegram. Наружу уходит только она, состояние DOM остаётся внутри. */
  onChange: (html: string) => void
  placeholder?: string
  /** Меняется после отправки — очищает поле, не трогая каретку при обычной правке. */
  resetKey: number
}
...
/**
 * Редактор рассылки: что набрано, то и видно.
 *
 * Раньше здесь была textarea, и админ писал теги руками — в поле стояло
 * `<b>Заголовок</b>`, а как это будет выглядеть, выяснялось только после
 * отправки. Показать жирный текст в textarea невозможно в принципе, поэтому
 * поле стало contenteditable, а разметка Telegram собирается из его DOM.
 *
 ...
 * разметка, а очистка после отправки приходит сменой resetKey.
 */
export function BroadcastTextEditor({ onChange, placeholder, resetKey }: BroadcastTextEditorProps) {
  ...
  useEffect(() => {
    ...
    node.innerHTML = ''
    onChange('')
    // onChange в зависимостях дал бы очистку поля на каждый ререндер родителя.
  ...
  return (
    <div>
      ...
      <div
        ...
        role="textbox"
        aria-multiline="true"
        aria-label={t('admin.broadcast.compose')}
