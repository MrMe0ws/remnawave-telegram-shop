...
| `CABINET_DECOR_THEME`                | Декоративное оформление кабинета (`off` по умолчанию; цветовые: `green`, `pink`, `orange`, `yellow`, `violet`, `slate`; атмосферные/сезонные: `neon`, `new_year`, `summer`, `halloween`, `valentine`, `spring`, `black_friday`, `aurora`, `ocean`, `cyber`, `sunset`, `lavender`). Можно менять в админке (Оформление кабинета) без перезапуска; в bootstrap — поле `decor_theme` |
