...
- **После успешной оплаты:** начисление XP — `XPRubEquivalentForPurchase` (`internal/loyalty/pricing.go`): сумма в ₽ или Stars×`RUB_PER_STAR`, затем при нуле резерв по длительности подписки (`LOYALTY_XP_FALLBACK_BY_MONTH`), затем минимум (`LOYALTY_XP_MIN_PER_PURCHASE`), плюс опционально бонус за слоты Extra HWID (`LOYALTY_XP_BONUS_PER_EXTRA_HWID_SLOT`). Для Stars при нулевом курсе суммовой вклад в XP может обнуляться — тогда срабатывают резервные правила из env.
...
| `LOYALTY_XP_BONUS_PER_EXTRA_HWID_SLOT` | Добавка XP за каждый доп. слот Extra HWID в строке покупки (0 = выкл.) |
