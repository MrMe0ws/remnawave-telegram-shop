...
| `RUB_PER_STAR`                       | Сколько рублей условно «стоит» одна Telegram Star (`float`, пусто/`0` = не использовать). В `tariffs`: расчёт Stars при оплате, если в БД нет `amount_stars`; подсказки в админке |
