...
| `FORTUNE_*`                          | **Колесо фортуны (кабинет):** `FORTUNE_ENABLED`; **`FORTUNE_MAX_SPINS_PER_DAY`** — максимум платных спинов за UTC-сутки (бесплатный daily в счёт не входит); `FORTUNE_SPIN_COST_DAYS`; `FORTUNE_MIN_SUBSCRIPTION_DAYS`; **`FORTUNE_DAILY_FREE_SPIN`** — один бесплатный спин в сутки (UTC), миграция **`000034`** (`is_daily_free`); **веса** `FORTUNE_WEIGHT_*`; **величины** `FORTUNE_REWARD_*`. Доступ: хотя бы одна оплаченная подписка (`month>0`). Конфиг: `internal/cabinet/config/fortune_wheel.go`. Миграции: **`000033`**, при daily — **`000034`**. |
