...
## 7) Плавный запуск (без даунтайма)

- Сначала выкатите версию с миграциями и `CABINET_ENABLED=false`.
- Проверьте, что бот работает стабильно.
- Затем включите `CABINET_ENABLED=true` и перезапустите только bot-сервис.
- После включения кабинета проверьте auth/bootstrap и web-login.
