...
Собранные файлы попадают в `internal/cabinet/web/dist/` и **должны быть закоммичены** (или пересобраны в CI перед `go build`), иначе в бинарнике не окажется актуального JS/CSS и SPA не откроется.
